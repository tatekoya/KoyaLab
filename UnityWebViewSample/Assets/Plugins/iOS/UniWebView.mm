...
extern "C" {
    ...
    const char * _UniWebViewGetCookie(const char *url, const char *key);
    void _UniWebViewSetCookie(const char *url, const char *cookie);
    ...
    void _UniWebViewSetAllowAutoPlay(const char *name, BOOL allowed);
    void _UniWebViewSetAllowInlinePlay(const char *name, BOOL allowed);
}
...
const char * _UniWebViewGetCookie(const char *aUrl, const char *aKey) {
    NSString *url = UniWebViewMakeNSString(aUrl);
    NSString *key = UniWebViewMakeNSString(aKey);
    
    NSURL *targetURL = [NSURL URLWithString:url];
    if (!targetURL) {
        NSLog(@"Invalid url: %@", url);
        return UniWebViewMakeCString(@"");
    }
    
    NSArray* cookies = [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookiesForURL:targetURL];
    for (NSHTTPCookie *cookie in cookies) {
        if ([cookie.name isEqualToString:key]) {
            return UniWebViewMakeCString(cookie.value);
        }
    }
...
void _UniWebViewSetCookie(const char *aUrl, const char *aCookie) {
    NSString *url = UniWebViewMakeNSString(aUrl);
    NSString *cookieString = UniWebViewMakeNSString(aCookie);
    ...
    NSDictionary *header = @{@"Set-Cookie": cookieString};
    NSArray *cookies = [NSHTTPCookie cookiesWithResponseHeaderFields:header forURL:targetURL];
    [[NSHTTPCookieStorage sharedHTTPCookieStorage] setCookies:cookies forURL:targetURL mainDocumentURL:nil];
}
...
void _UniWebViewSetAllowAutoPlay(const char *name, BOOL allowed) {
    UniWebView *webView = [[UniWebViewManager sharedManager] webViewWithName:UniWebViewMakeNSString(name)];
    webView.mediaPlaybackRequiresUserAction = !allowed;
}

void _UniWebViewSetAllowInlinePlay(const char *name, BOOL allowed) {
    UniWebView *webView = [[UniWebViewManager sharedManager] webViewWithName:UniWebViewMakeNSString(name)];
    webView.allowsInlineMediaPlayback = allowed;
}
